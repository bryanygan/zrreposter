...
test('parseSince parses durations relative to now', () => {
  const now = Date.parse('2026-07-10T00:00:00Z');
  assert.strictEqual(parseSince('50h', now), now - 50 * 3600000);
  assert.strictEqual(parseSince('2 days', now), now - 2 * 86400000);
  assert.strictEqual(parseSince('90 min', now), now - 90 * 60000);
  assert.strictEqual(parseSince('1w', now), now - 7 * 86400000);
});

test('parseSince parses dates as Eastern midnight (EDT in summer)', () => {
  const now = Date.parse('2026-07-10T12:00:00Z');
  // July -> EDT (UTC-4), so midnight ET is 04:00 UTC.
  assert.strictEqual(parseSince('07/07', now), Date.UTC(2026, 6, 7, 4));
  assert.strictEqual(parseSince('2026-07-07', now), Date.UTC(2026, 6, 7, 4));
  assert.strictEqual(parseSince('7/7/2025', now), Date.UTC(2025, 6, 7, 4));
});

test('parseSince honors DST (EST in winter)', () => {
  const now = Date.parse('2026-01-20T12:00:00Z');
  // January -> EST (UTC-5), so midnight ET is 05:00 UTC.
  assert.strictEqual(parseSince('01/15/2026', now), Date.UTC(2026, 0, 15, 5));
});

test('parseSince accepts an explicit timezone', () => {
  const now = Date.parse('2026-07-10T12:00:00Z');
  assert.strictEqual(parseSince('07/07', now, 'UTC'), Date.UTC(2026, 6, 7));
});

test('parseSince returns null for empty input', () => {
  assert.strictEqual(parseSince('', 1), null);
  assert.strictEqual(parseSince(null, 1), null);
});

test('parseSince throws on unrecognized or impossible input', () => {
  assert.throws(() => parseSince('banana', 1), /Could not understand/);
  assert.throws(() => parseSince('02/30', 1), /Could not understand/);
});
