...
// request to an attacker-chosen host; the share page serves its HTML directly.
// The body is size-capped to bound memory use.
async function resolveTaobaoShareLink(url, fetchImpl) {
  ...
  try {
    ...
    return html ? normalizeGoofishLinks(html) : [];
  } catch {
    return [];
  } finally {
    clearTimeout(timer);
